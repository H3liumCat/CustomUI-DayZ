...
class CustomLoadingScreenData
{
	...
	string HintIconPath = "CustomUI/GUI/icons/circle_info.edds";
	...
	void CustomLoadingScreenData()
	{
		...
		JsonFileLoader<array<ref CustomLoadingScreenBackground>>.JsonLoadFile("CustomUI/Scripts/Data/LoadingImages.json", m_ExBackgrounds);
	}
};

modded class UiHintPanel
{
	protected const string 	m_ExDataPath = "CustomUI/Scripts/Data/Hints.json"; //! Custom hints json path
...
modded class LoadingScreen
{
	...
	void LoadingScreen(DayZGame game)
	{
		...
		m_ExHintIcon = ImageWidget.Cast(game.GetLoadingWorkspace().CreateWidgets("CustomUI/GUI/layouts/hint_icon.layout", m_ExPanelWidget));
		...
		if (m_ExCustomLoadingScreenData.LoadingBarColor != 0)
		{
			...
			m_ProgressLoading = ProgressBarWidget.Cast(game.GetLoadingWorkspace().CreateWidgets("CustomUI/GUI/layouts/loading_bar.layout", m_ExPanelWidget));
